...
final class DetailAction
{
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        if (file_exists($filePath) && ((time() - json_decode(utf8_decode(file_get_contents($filePath)))->expireTime * 60) < filemtime($filePath))) {
            $results = json_decode(utf8_decode(file_get_contents($filePath)));
            $url = $results->url;
            $expireTime = $results->expireTime;

            if ((time() - json_decode(utf8_decode(file_get_contents($filePath)))->expireTime * 60) < filemtime($filePath)) {
                $this->logger->info("Detail page action dispatched " . json_decode(utf8_decode(file_get_contents($filePath)))->expireTime );
            }
